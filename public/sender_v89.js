...
function loadBlocksToPattern(blocks) {
  ...
  for (let i = 0; i < 5472; i++) {
    ...
    // Stateless hash PRNG prevents subpixel optical aliasing and guarantees sync
...
function runLoopbackTest() {
  ...
  try {
    ...
    if (frame.valid) {
      ...
      status.textContent = `CRC OK! "${decoded.substring(0, 20)}" err=${frame.errorsCorrected} rB=${senderHex}`;
      status.style.color = '#0f0';
    } else {
      status.textContent = `CRC LOOPBACK FAILED! ch=${frame.failedChannel} rB=${senderHex}`;
      status.style.color = '#f00';
    }
  } catch (e) {
    status.textContent = `CRC EXCEPTION: ${e.message}`;
    status.style.color = '#f00';
  }
}
